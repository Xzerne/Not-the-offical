...
local Window = redzlib:MakeWindow({
  Title = "Quincy Hub",
  SubTitle = "By _toshun On Discord",
  SaveFolder = "QuincyHub | By _toshun"
})

local Cre = Window:MakeTab({"Credits", ""})
local Main = Window:MakeTab({"Main Farm", ""})
local Item= Window:MakeTab({"Sub Farm", ""})
local Ma= Window:MakeTab({"Materials Farm", ""})
local P = Window:MakeTab({"Pvp Players", ""})
local Shop = Window:MakeTab({"Items Buyer", ""})
local Fruit = Window:MakeTab({"Fruit-Visual", ""})
local Tp = Window:MakeTab({"Traveling", ""})
local Raid = Window:MakeTab({"Raid", ""})
local Race = Window:MakeTab({"Race V4-Mirage", ""})
local Set = Window:MakeTab({"Settings", ""})
local SV = Window:MakeTab({"Server Status", ""})
local PST = Window:MakeTab({"Player Status", ""})
local Mi = Window:MakeTab({"Misc-CFrame", ""})

--Credits
Cre:AddDiscordInvite({
  Name = "Quincy Hub | Community",
  Logo = "",
  Invite = "None"
})


local Paragraph = Cre:AddParagraph({"Developer", "By _toshun On Discord"})

local Section = Main:AddSection({"Main Farm"})
...
local Dropdown = Main:AddDropdown({
  ...
  Options = farmode,
  Default = "",
  Flag = "Select Farm Mode",
  Callback = function(Value)
    FarmMode = Value
  end
})
    
    local LevelFarmTog = Main:AddToggle({
  Name = "Farm Level",
  Description = "1-2550",
  Default = false
})

LevelFarmTog:Callback(function(Value)
  _G.AutoFarm = Value
...
spawn(function()
    ...
    local Section = Main:AddSection({"Mobs"})
...
local FarmMobTog = Main:AddToggle({
  Name = "Farm Mobs Selected",
  Description = _G.SelectMob,
  Default = false
})

FarmMobTog:Callback(function(Value)
...
spawn(function()
    ...
    local FarmNearTog = Main:AddToggle({
  Name = "Farm Mobs Selected",
  Description = _G.SelectMob,
  Default = false
})

FarmNearTog:Callback(function(Value)
...
spawn(function()
    ...
    local Section = Main:AddSection({"Dough Boss"})
    
    
    function killedmobsdough()
    spawn(function()
    while wait() do
    pcall(function()
    if string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")) == 88 then
    return "Defeat : "..string.sub(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"),39,41)
    elseif string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")) == 87 then
                   return "Defeat : "..string.sub(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"),39,40)
                elseif string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")) == 86 then
                    return "Defeat : "..string.sub(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner"),39,39)
                else
    ...
    local Paragraph = Main:AddParagraph({"Killed : ", killedmobsdough()})
    
    local ListC = {"No Quest", "Quest"}
    CakeFMode = "Quest"
    local Dropdown = Main:AddDropdown({
  Name = "Select Katakuri Farm Spawn Mode",
  Description = "",
  Options = tableMon,
  Default = "",
  Flag = "Dough Mode Select",
  Callback = function(Value)
    CakeFMode = Value
  end
})

    local FarmMobDoughTog = Main:AddToggle({
  Name = "Farm To Spawn Katakuri",
  Description = "",
  Default = false
})

FarmMobDoughTog:Callback(function(Value)
...
spawn(function()
    ...
    local spawndoughtog = Main:AddToggle({
  Name = "Auto Spawn Katakuri",
  Description = "Spawn boss when killed enough mobs",
  Default = true
})

spawndoughtog:Callback(function(Value)
  game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner",Value)
end)

local autoktv2tog = Main:AddToggle({
  Name = "Auto Katakuri V2",
  Description = "Need Spawn",
  Default = true
})

autoktv2tog:Callback(function(Value)
